diff --git a/PC_code.py b/PC_code.py
index 8b137891791fe96927ad78e64b0aad7bded08bdc..6a7d341b9cd097e581580c064a0cc7f78da56b45 100644
--- a/PC_code.py
+++ b/PC_code.py
@@ -1 +1,310 @@
+"""
+frs_ble_monitor.py
+Conecta por BLE (Nordic UART Service) a XIAO-FRS, recibe "t_ms,adc",
+filtra (0.2–5 Hz), detecta respiraciones y muestra RPM en tiempo real.
...
+CLIP_HI = 4045
+
+ble_loop = None  # event loop en hilo BLE
+
+
...
+        self.t_abs = deque(maxlen=self.maxlen)
...
+                if len(self.peak_times) == 0 or (t_now - self.peak_times[-1]) > PEAK_REFRACTORY_S:
...
+
+    def update_quality(self, t_rel, y_raw_vis, y_flt_vis):
+        if len(y_flt_vis) < 20:
...
+            state = "Señal baja"
+        elif pp > 1200:
+            state = "Riesgo de clip"
+        else:
+            state = "OK"
+        self.quality_label.setText(f"Calidad: {state} | p-p: {pp:.0f} cuentas | clip: {'sí' if clipped else 'no'}")
+
...
+
+        self.update_quality(t_rel, y_vis, yf_vis)
+
+
+# ---------- Hilo BLE (asyncio) ----------
+async def ble_connect_and_stream(app: FRSApp):
+    while True:
+        device = None
...
+        if device is None:
+            for d in devices:
+                if NUS_SERVICE.lower() in [s.lower() for s in d.metadata.get("uuids", [])]:
+                    device = d
+                    break
+        if device is None:
+            print("XIAO-FRS no encontrado. Reintentando en 3 s.")
...
+            print(f"BLE desconectado: {e}. Reintentando en 2 s.")
...
+        ble_loop.run_until_complete(ble_connect_and_stream(app))
